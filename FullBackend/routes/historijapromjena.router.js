...
router.put("/historijapromjena/:id", (req, res, next) => {
    let id = req.params.id;
    const { datumOd, datumDo, promjenaId, statusPromjeneId, napravioIzmjenu } = req.body;
    HistorijaPromjena.update({ datumOd, datumDo, promjenaId, statusPromjeneId, napravioIzmjenu }, { where: { id }, attributes: ["datumOd", "datumDo", "promjenaId", "statusPromjeneId", "napravioIzmjenu"], raw: true })
